...
{	SF_PRIVATE 		*psf = NULL ;
	...
	{	case SFC_GET_LIB_VERSION :
			...
			if (ENABLE_EXPERIMENTAL_CODE)
				LSF_SNPRINTF ((char*)data, datasize, "%s-%s-exp", PACKAGE_NAME, PACKAGE_VERSION) ;
			else
				LSF_SNPRINTF ((char*)data, datasize, "%s-%s", PACKAGE_NAME, PACKAGE_VERSION) ;
			return 0 ;
	...
	{	if (data == NULL)
			return (psf->error = SFE_BAD_CONTROL_CMD) ;
		LSF_SNPRINTF ((char*)data, datasize, "%s", sf_logbuffer) ;
		return 0 ;
		} ;
	...
	{	case SFC_SET_NORM_FLOAT :
		...
		case SFC_GET_LOG_INFO :
			if (data == NULL)
				return (psf->error = SFE_BAD_CONTROL_CMD) ;
			LSF_SNPRINTF ((char*)data, datasize, "%s", psf->logbuffer) ;
			break ;
