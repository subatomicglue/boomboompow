...
#include <fstream> // this exporter write files using std::ofstream
...
{
   #ifdef WIN32
   std::ofstream hBitmapFile( filename, std::ios::out | ios::binary );
   #else
   std::ofstream hBitmapFile( filename, std::ios::out );
   #endif
   ...
   */
   ...
   for (int k = 0; k < image.height(); ++k)
   {
      ...
      hBitmapFile.write( (char *)bgrImage.data(), image.width() * 3 );
      ...
      hBitmapFile.write( (char *)&byte, padsize );
   }
