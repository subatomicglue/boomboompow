...
{
#ifdef VERBOSE_KEV
    printf( "__debug_new_recorder::_M_process %s %d\n", _M_file, _M_line );
#endif
    if (pointer == NULL)
        return;
...
{
#ifdef VERBOSE_KEV
    printf( "new(size_t size, const char* file=%s, int line=%d)\n", line==0?"unknown":file, line );
#endif
...
{
#ifdef VERBOSE_KEV
   printf( " new(size_t size) throw(std::bad_alloc)\n" );
#endif
...
{
#ifdef VERBOSE_KEV
   printf( "new(size_t size, const std::nothrow_t&) throw()\n" );
#endif
