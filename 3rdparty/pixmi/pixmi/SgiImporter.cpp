...
bool SgiImporter::load( const char* const filename, Image& image )
{
       ...
       strstr(filename, ".rgba") || strstr(filename, ".bw") ) 
   {
      ...
      #ifdef WIN32
      std::ifstream hRGBFile( filename, std::ios::in | std::ios::binary );
      #else
      std::ifstream hRGBFile( filename, std::ios::in );
      #endif
      
      // why would std::ifstream mangle this pointer?  that violates const-ness. (you probably need to recompile clean)
      assert( filename != NULL && "This should never happen, std::ifstream's constructor mangled the filename address" );
   ...
   //Log(0)
      std::cout << filename << ": Specify an rgb image type (extention must be sgi/rgb/rgba/bw)\n" << std::flush;
   return false;
}
...
bool SgiImporter::LoadData(std::ifstream &file, Image& image)
{
   if(_STORAGE == RGB_RLE)
...
//: Read the SGI image header from an std::ifstream
bool SgiImporter::LoadHeader(std::ifstream &file)
{
     char filler[404];
   file.read((char *)&_MAGIC,   sizeof(unsigned short));
   file.read((char *)&_STORAGE,   sizeof(unsigned char) );
   file.read((char *)&_BPC,   sizeof(unsigned char) );
   file.read((char *)&_DIMENSION,   sizeof(unsigned short));
   file.read((char *)&_XSIZE,   sizeof(unsigned short));
   file.read((char *)&_YSIZE,   sizeof(unsigned short));
   file.read((char *)&_ZSIZE,   sizeof(unsigned short));
   file.read((char *)&_PIXMIN,   sizeof(unsigned long) );
   file.read((char *)&_PIXMAX,   sizeof(unsigned long) );
   file.read((char *) filler,   sizeof(unsigned long) );
   char IMAGENAME[80];
   file.read((char *) IMAGENAME,   sizeof(unsigned char) * 80);
   file.read((char *)&_COLORMAP,   sizeof(unsigned long) );
   file.read((char *) filler,   sizeof(unsigned char) * 404);
   ...
   if (_MAGIC != RGB_MAGIC)
   {
      std::cout<<"Not an rgb image according to the magic number. "<<_MAGIC<<"!="
         <<RGB_MAGIC<<"\n"<<std::flush;
      std::cout<<IMAGENAME<<" is not SGI format\n"<<std::flush;
      return false;
   } 
   ...
   if (_DIMENSION == 1 && 
      (_ZSIZE != 1 || _YSIZE != 1) )
   {
      std::cout<<"Error: Dimension is 1, you cannot have more than one color, or scanline\n"
         <<std::flush;
      std::cout<<" - Dimensions ="<<_XSIZE<<"x"<<_YSIZE<<":"<<_ZSIZE<<" colors\n"<<std::flush;
      return false;
   }

   if (_DIMENSION == 2 && _ZSIZE != 1 )
   {
      std::cout<<"Error: Dimension is 2, you cannot have more than one color\n"<<std::flush;
      std::cout<<" - Dimensions ="<<_XSIZE<<"x"<<_YSIZE<<":"<<_ZSIZE<<" colors\n"<<std::flush;
      return false;
   } 
   ...
   {
      std::cout<<"Error: color channel ranges from "<<_PIXMIN<<" to "<<_PIXMAX<<"\n"<<std::flush;
      std::cout<<"       this conflicts with the total number of colors per channel\n"<<std::flush;
      std::cout<<"       specified in the header.  BitsPerChannel = "<< twoToTheBPC8 <<"\n"<<std::flush;
      return false;
   }

   if (_COLORMAP == RGB_SCREEN || _COLORMAP == RGB_DITHERED)
   {
      std::cout<<"Error: this rgb format is obsolete, don't use screen or "
         <<"dithered encoding\n"<<std::flush;
      return false;
   }
...
//: read from an std::ifstream RLE data
// header will tell you if the data is RLE or RGB encoded
bool SgiImporter::LoadRLE(std::ifstream &file, Image& image)
{
    ...
    {
      file.read((char *) &startTable[y], sizeof(unsigned long));
    ...
    for (y = 0; y < scanlines; y++)
    {
       file.read((char*) &lengthTable[y], sizeof(unsigned long));
       // SGI format is big endian, 
       // so will need to swap bytes if system architecture is little endian.
       if (pixmi::isLittleEndian())
       {
   ...
   rleData = (unsigned char *)new unsigned char[rleLength];
    file.read((char *)rleData, rleLength);
...
//: read from an std::ifstream RGB data
// header will tell you if the data is RLE or RGB encoded
bool SgiImporter::LoadRGB(std::ifstream &file, Image& image)
{
    int x, y, channel;
    ...
    file.read((char *)verbatimData, rgbLength);
