...
public:
  inline Vector2d() {}
  inline Vector2d( float x, float y ) { v[0] = x; v[1] = y; }
  inline Vector2d( const Vector2d& v ) { copy( v ); }
  inline float operator[]( int x ) const { return v[x]; }
  inline void copy( const Vector2d& vec )
  {
     v[0] = vec.v[0];
     v[1] = vec.v[1];
     tv[0] = vec.tv[0];
